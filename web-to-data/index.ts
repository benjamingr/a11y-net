...
(async () => {
  ...
  for await (const page of getParsedContent(path.join(__dirname, './10k_results.json'), x =>
    ...
    const others = shuffle(
      Array.from(page.dom.querySelectorAll('body > * > * > :not([role=button]')),
    );
...
function getFeatures(element: Element, page: Document) {
  ...
  return {
    ...
    descendentsClassList: uniq(
      Array.from(element.querySelectorAll('*'), x => x.classList)
        .flat()
        .map(x => x.value.split(' '))
        .flat(),
    ).join(' '),
    tagNamesWithin: Array.from(
      new Set(Array.from(element.querySelectorAll('*'), x => x.tagName.toUpperCase())),
    )
      .slice(0, 10)
      .join(' '),
  };
